...
__all__ = ['EMA', 'SimAM', 'SpatialGroupEnhance', 'BiLevelRoutingAttention', 'BiLevelRoutingAttention_nchw', 'TripletAttention', 
           'CoordAtt', 'BAMBlock', 'EfficientAttention', 'LSKBlock', 'SEAttention', 'CPCA','CPDCA', 'MPCA', 'deformable_LKA',
           'EffectiveSEModule', 'LSKA', 'SegNext_Attention', 'DAttention', 'FocusedLinearAttention', 'MLCA', 'TransNeXt_AggregatedAttention','ResNetLayer']
...
class CPDCA(nn.Module):
    def __init__(self, channels, channelAttention_reduce=4):
        ...
        self.dconv3_31 = nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels)
        self.dconv3_32 = nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels)

        self.dconv1_1 = nn.Conv2d(channels, channels, kernel_size=3, dilation=1, padding=1, stride=1, groups=channels)
        self.dconv1_2 = nn.Conv2d(channels, channels, kernel_size=3, dilation=2, padding=2, stride=1, groups=channels)
        self.dconv1_3 = nn.Conv2d(channels, channels, kernel_size=3, dilation=3, padding=3, stride=1, groups=channels)
    ...
    def forward(self, inputs):
        ...
        x_init = self.dconv3_31(inputs)
        x_init = self.dconv3_31(x_init)
        x_1 = self.dconv1_1(x_init)
        x_1 = self.dconv1_2(x_1)
        x_1 = self.dconv1_3(x_1)
        x_2 = self.dconv1_7(x_init)
        x_2 = self.dconv7_1(x_2)
        x_3 = self.dconv1_21(x_init)
        x_3 = self.dconv21_1(x_3)
        x = x_1 + x_3 + x_2 + x_init
